...
if(strstr($item->rendered_fields['field_status'],'Available')){
  $bg_color = variable_get('sfsu_booking_color_avail', '#e0ffbd');
}
else{
  $bg_color = variable_get('sfsu_booking_color_other', '#f3e891');
}
...
<div class="<?php print !empty($item->class) ? $item->class : 'item'; ?>">
  ...
  <div class="calendar <?php print $item->granularity; ?>view" style="background-color:<?php print $bg_color; ?>" >
    <div class="calendar-item <?php print $item->date_id; ?> contents">
      <?php // produce a flag link
/*            $flag = flag_get_flag('appointment_booking');
              if (!$flag->is_flagged($item->id) && user_is_logged_in()){
                print '<div class="flag-link appointment-booking"><b>-</b>' . flag_create_link($flag->name, $item->id) . '</div>'
      } 
*/      ?>
